...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        ch_i,ch_o = 1,1
        ch_h = 64
        seq.append(torch.nn.Conv3d(in_channels=ch_i, out_channels=ch_h, kernel_size=(3, 3, 3),
                                   stride=(1, 1, 1), padding=(1, 1, 1)))
        for _ in range(5):
            seq.append(torch.nn.Conv3d(in_channels=ch_h,out_channels=ch_h,kernel_size=(3,3,3),
                                       stride=(1,1,1),padding=(1,1,1)))
            seq.append(torch.nn.BatchNorm3d(ch_h))
            seq.append(torch.nn.ReLU())
        seq.append(torch.nn.Conv3d(in_channels=ch_h, out_channels=ch_o, kernel_size=(3, 3, 3),
                                   stride=(1, 1, 1), padding=(1, 1, 1)))
        seq.append(torch.nn.ReLU())
        self.cnn_sequence = torch.nn.Sequential(*seq)

    def forward(self, x):
        y = self.cnn_sequence(x[None,None,:,:,:])
        return y[0,0,:,:,:]

def main():
    print(args)

    spect = SPECT_system_torch(projections_fn=args.projections,like_fn=args.input,geom_fn=args.geom)
    ...
    input_tensor = torch.tensor(spect.like_array, device=device)
    input_max = input_tensor.max()
    input_tensor = input_tensor/input_max
    ...
    for iteration in range(1,args.niter+1):
        ...
        input_tensor_corrected = model(input_tensor) * input_max
        fp_image_k_tensor = forwardprojection.apply(input_tensor_corrected, spect) # computes forward-projection of current estimate
        ...
        if (args.output_every is not None) and (iteration%args.output_every==0):
            image_k_array = input_tensor_corrected.detach().cpu().numpy()
